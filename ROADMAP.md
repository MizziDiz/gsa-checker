...
### 4c. Недельная статистика по странам (`--report`)  ✅ ГОТОВО (замена split1404)
Делает то же, что ручной `Split/split1404.py`, но источник — verified-CSV из GSA (колонки
`Country` по IP-геолокации GSA, `URL`, `IP`). Раскладка по бакетам логикой split1404 1:1
(`lib/buckets.py`: `COUNTRY_FILES`/`REGION_FILES`/`SUMMARY_ORDER`), **инкрементная запись
только новых URL в базу `out_country_buckets` (`buckets_dir`)** с дедупом per-file/global/
in-run, добор `Not Stated` по IP через GeoIP (без DNS). Сводка формата `debug_summary.txt`
(`Страна ВСЕГО (+новых) … Не указано … ИТОГО`) → `report_out_dir` + Telegram. **Проверено:
вывод байт-в-байт == split1404 на реальной базе (501 новых из 16 112); 2-й прогон = +0
(идемпотентно).** `--dry-run` считает без записи. Оператор по сводке решает, сколько
добрать; сам добор — их `select_links_by_targets`. Расписание — планировщик Windows
`/SC WEEKLY /D MON` (по понедельникам), команда `--ui-export --report`.
```
...
Автоматизирует ручной шаг выгрузки (фича C). Ручной путь (подтверждён оператором,
