...
### 6. Автопилот  ✅ ГОТОВО (общий пул, дозаливка)
`--autopilot`: общий пул — при остатке ниже `autopilot_min_targets` берёт новейший
неиспользованный батч из `autopilot_pool_dir` (`for_gsa_ser`) и ДОПИСЫВАЕТ его цели в
файл `autopilot_append_ext` (`.new_targets`) проектов ниже порога, **не стирая данные**.
Идемпотентно по журналу `data/gsa_autopilot.jsonl` (батч→проект): один батч в проект
один раз, новейший первым, один батч за прогон. Сухой прогон по умолчанию, `--apply`
пишет + шлёт Telegram (с напоминанием обновить интерфейс GSA). НЕ создаёт новые
проекты (это автогенератор для «особых» — отдельно).
```
