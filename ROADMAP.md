...
### 4c. Статистика по странам для добора (`--report`)  ✅ ГОТОВО
Читает verified-CSV из GSA (колонки `Country` по IP-геолокации GSA + `IP`), бакетизирует
логикой Split 1:1 (`lib/buckets.py`: `COUNTRY_BUCKETS`+`REGION_RULES`), добивает
`not_stated` по IP через GeoIP (без DNS — IP в CSV), считает по бакетам, пишет отчёт в
`report_out_dir` и шлёт сводку в Telegram. Оператор по сводке говорит, сколько добрать;
сам добор — их `select_links_by_targets`. Проверено на 16 112 строках (36 бакетов).
```
