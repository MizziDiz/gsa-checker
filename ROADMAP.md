...
### 3. `--create` — создание проекта из шаблона + заливка целей
Главный сценарий: батч из `\\share\for_gsa_ser\<дата>\*.txt` → проект GSA.
- [ ] шаблон проекта (эталонный `.prj` с нужными движками/настройками) → клонируем,
      подставляем имя/ключи/список целей;
- [ ] заливка целевого списка в кэш целей проекта (формат/файл — по образцу);
- [ ] идемпотентность: не создавать дубль по уже обработанному батчу (журнал, как
      `aparser_sent.jsonl`);
- [ ] `--create --dry-run`: показать, что будет создано, без записи;
- [ ] перечитывание проектов GSA: файлово (импорт/рестарт) или через `lib/ui.py`.

### 4. `lib/ui.py` — UI-автоматизация (живые правки настроек)
Для запущенного GSA, где файловый путь не подходит (правка на лету, старт/стоп).
- [ ] выбор технологии: pywinauto (нативно из Python) vs AutoIt-скрипты;
- [ ] базовые операции: найти окно, список проектов, старт/стоп проекта, открыть
      настройки, изменить поле, применить;
- [ ] хрупкость к версии: селекторы/раскладку вынести в конфиг, `--ui-check` (сухой
      прогон с логом того, что нашлось), как `--check` у A-Parser UI.
...
### 6. Автопилот
Простой GSA (мало целей) → взять следующий батч из `for_gsa_ser` → `--create`.
Аналог `--autopilot` в Aparser-checker; идемпотентность и лимит новых проектов.
