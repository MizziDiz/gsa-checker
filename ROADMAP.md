...
### 4c. Недельная статистика по странам (`--report`)  ✅ ГОТОВО (замена split1404, БЕЗ UI)
Делает то же, что ручной `Split/split1404.py`. **Источник по умолчанию — файлы `.success`
проектов на диске** (GSA не трогаем). Открыто на реальных данных: GSA сам пишет в
`.success` код страны (предпосл. поле, ISO2) и IP (посл. поле); страна = ccTLD, для gTLD —
по IP. Сверка на 1.csv: ccTLD ↔ страна GSA = **100%** (313/313), прямой GeoIP-по-IP = 45%
(IP это хостинг). Значит берём страну прямо из `.success` = страна GSA 1:1. `--report`:
`.success` (`verified_glob`/`gsa_projects_dir`) → ISO2→имя (`lib/iso2.py`) → бакеты
split1404 1:1 (`lib/buckets.py`) → **инкремент только новых URL в базу
`out_country_buckets` (`buckets_dir`)** (дедуп per-file/global/in-run) → пустые коды добор
по IP через GeoIP → сводка формата `debug_summary.txt` в `report_out_dir` + Telegram.
Источник `.success` разбирается генератором `_iter_verified` (16 полей через 0xFF).
**Проверено: формат 1:1 со split1404, инкремент идемпотентен (+64, повтор +0).** `--dry-run`
без записи. Оператор по сводке решает, сколько добрать; сам добор — их
`select_links_by_targets`. Расписание — планировщик Windows `/SC WEEKLY /D MON`, команда
`--report`.
```
python gsa_checker.py --report               # штатно, из .success, без UI
python gsa_checker.py --report --dry-run      # без записи в базу
python gsa_checker.py --report --csv <файл>   # альт. источник — GSA verified CSV
```
### 4d. (Опционально) UI-выгрузка / сверка — `--ui-export` / `--geocheck`
Больше НЕ нужны штатно (`.success` самодостаточен), остаются как инструменты.
**`--ui-export`** (каркас): автоматизирует ручной Create Report через UI — выделить все →
ПКМ → Modify Project → Export → Create Report → CSV → сохранить; клавишами `^a` →
`{VK_APPS}` → `{UP}{UP}{RIGHT}{DOWN}×6{RIGHT}{UP}{ENTER}` → `{ENTER}` → диалог (класс
`#32770`). Хрупко (клики по меню, рядом Delete/Reset Data), нужна живая обкатка.
**`--geocheck --csv <файл>`**: read-only сверка страны GSA vs наш GeoIP по IP — ей и
выяснили, что GSA считает по ccTLD, а не по IP.
...
`--autopilot`: заливает цели **ТОЛЬКО в проекты с именем из `autopilot_include_names`
(по умолч. `Split`)**, дополнительно режет `autopilot_exclude_names` (`CC`/`TEST`/`Common`)
— фильтр по имени, т.к. `last status` в `.prj` не различает active/inactive. Белый список
защищает от того, что новый посторонний проект начнёт кормиться сам собой.
**Асимметрия (правило оператора):** кормим только `Split`, а verified читаем со ВСЕХ
проектов (`--report` по имени не фильтрует) — неактивные ссылок не добавят, но их база
нужна в статистике. При остатке любого проекта ниже
