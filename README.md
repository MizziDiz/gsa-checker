...
`[email accounts]` (как `--emails`). `[Options]`/`[engines]` — байт-в-байт не трогает.
```
...
```
`respin_fields` в конфиге сузит набор полей (по умолчанию — все спин-поля). `--count` —
почт на проект. ⚠ Как `--settings`/`--emails`: при **закрытом GSA**.
